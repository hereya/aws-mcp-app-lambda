...
function loginPage(returnUrl, error, customCss, lang) {
  const s = t(lang || "en");
  return `<!doctype html>
<html lang="${lang || "en"}">
<head>
...
<body>
  <div class="card">
    ...
    <form method="POST" action="send-otp">
      ...
      <input type="email" id="email" name="email" required autofocus placeholder="${s.emailPlaceholder}">
      <button type="submit">${s.continueButton}</button>
    </form>
  </div>
</body>
</html>`;
...
function otpPage(session, email, returnUrl, error, customCss, lang) {
  const s = t(lang || "en");
  return `<!doctype html>
<html lang="${lang || "en"}">
<head>
...
<body>
  <div class="card">
    ...
    <form method="POST" action="verify">
      ...
      <button type="submit">${s.verifyButton}</button>
    </form>
  </div>
</body>
</html>`;
...
async function handleLogin(event) {
  ...
  return htmlResponse(200, loginPage(returnUrl, null, css, lang));
}
...
async function handleSendOtp(event) {
  ...
  try {
    ...
    return htmlResponse(200, otpPage(session, email, returnUrl, null, css, lang));
  } catch (err) {
    console.error("InitiateAuth error:", err);
    if (err.name === "UserNotFoundException" || err.name === "NotAuthorizedException") {
      return htmlResponse(200, loginPage(returnUrl, s.noAccount, css, lang));
    }
    return htmlResponse(500, loginPage(returnUrl, s.sendFailed, css, lang));
  }
}
...
async function handleVerify(event) {
  ...
  if (!session || !otp || !email) {
    return htmlResponse(400, otpPage(session || "", email || "", returnUrl, s.missingFields, css, lang));
  }
  ...
  try {
    ...
    if (result.Session) {
      return htmlResponse(
        200,
        otpPage(result.Session, email, returnUrl, s.incorrectCode, css, lang)
      );
    }

    return htmlResponse(500, loginPage(returnUrl, s.authFailed, css, lang));
  } catch (err) {
    console.error("RespondToAuthChallenge error:", err);
    return htmlResponse(
      200,
      otpPage(session, email, returnUrl, s.expiredSession, css, lang)
    );
  }
}
