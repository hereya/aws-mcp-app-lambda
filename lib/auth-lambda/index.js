...
async function loadCustomCss(app) {
  ...
  if (!bucket) { cssCache[app] = null; return null; }
...
function loginPage(returnUrl, error, customCss) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
function otpPage(session, email, returnUrl, error, customCss) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
async function handleLogin(event) {
  ...
  return htmlResponse(200, loginPage(returnUrl, null, css));
}
...
async function handleSendOtp(event) {
  ...
  if (!email) {
    return htmlResponse(400, loginPage(returnUrl, "Email is required.", css));
  }
  ...
  try {
    ...
    return htmlResponse(200, otpPage(session, email, returnUrl, null, css));
  } catch (err) {
    console.error("InitiateAuth error:", err);
    if (err.name === "UserNotFoundException" || err.name === "NotAuthorizedException") {
      return htmlResponse(200, loginPage(returnUrl, "No account found for this email. Contact the app owner to get access.", css));
    }
    return htmlResponse(500, loginPage(returnUrl, "Failed to send code. Please try again.", css));
  }
}
...
async function handleVerify(event) {
  ...
  if (!session || !otp || !email) {
    return htmlResponse(400, otpPage(session || "", email || "", returnUrl, "Missing required fields.", css));
  }
  ...
  try {
    ...
    if (result.Session) {
      return htmlResponse(
        200,
        otpPage(result.Session, email, returnUrl, "Incorrect code. Please try again.", css)
      );
    }

    return htmlResponse(500, loginPage(returnUrl, "Authentication failed.", css));
  } catch (err) {
    console.error("RespondToAuthChallenge error:", err);
    return htmlResponse(
      200,
      otpPage(session, email, returnUrl, "Incorrect code or session expired. Please try again.", css)
    );
  }
}
