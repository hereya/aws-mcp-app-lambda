...
// Derive the cookie Domain attribute from the Host header so the same auth
// flow works on default subdomains (`.${CUSTOM_DOMAIN}`) and user-bound vanity
// domains. For vanity domains we scope the cookie to the exact host so it
// doesn't leak to unrelated subdomains the user may also own.
