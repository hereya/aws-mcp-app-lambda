...
async function handleSendOtp(event) {
  ...
  try {
    // Ensure user exists in Cognito (auto-confirmed by PreSignUp trigger)
    try {
      ...
      // UsernameExistsException is expected for returning users
...
async function handleVerify(event) {
  ...
  try {
    ...
    if (result.AuthenticationResult?.IdToken) {
      ...
      // Build redirect path
      let redirectPath = returnUrl || "/view/home";
      if (app && redirectPath.startsWith("/")) {
...
async function handleLogout(event) {
  const app = extractApp(event.rawPath);
  const loginPath = app ? `/${app}/auth/login` : "/auth/login";
